...
import { Search, Camera, Package, X, ShoppingBasket, Banknote, CreditCard, Smartphone, Printer, Check, Trash2 } from "lucide-react";
      ...
      <div className="flex gap-4 h-[calc(100vh-80px)]">
        ...
        <div className="flex-[3] flex flex-col gap-4 min-w-0">
            ...
            <div className="relative flex-1">
              ...
              {search && searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 z-50 bg-card border rounded-md shadow-lg mt-1 max-h-64 overflow-y-auto">
                  {searchResults.map(p => (
                    <button key={p.id} className="w-full text-right px-4 py-3 hover:bg-muted flex justify-between items-center" onClick={() => addToCart(p)}>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
            {quickProducts.map(p => (
              <button key={p.id} onClick={() => addToCart(p)} className="flex flex-col items-center gap-1 p-2 rounded-lg border bg-card hover:bg-muted transition-colors text-center">
        ...
        {/* LEFT SIDE - Payment */}
        <div className="flex-[2] flex flex-col gap-4 min-w-[280px]">
          <Card className="flex-1 flex flex-col">
            ...
            <CardContent className="flex-1 flex flex-col gap-4">
              ...
              <div>
                ...
                <Input type="number" dir="ltr" className="h-12 text-xl text-center font-bold" value={paidAmount} onChange={e => setPaidAmount(e.target.value)} />
              </div>
              ...
              {paymentMethod === "cash" && (
                <div className="grid grid-cols-3 gap-2">
                  {quickAmounts.map(a => (
                    <Button key={a} variant="outline" size="sm" onClick={() => setPaidAmount(a.toString())} dir="ltr">₪{a}</Button>
                  ))}
                  <Button variant="outline" size="sm" onClick={() => setPaidAmount(totalAmount.toFixed(2))}>المبلغ بالضبط</Button>
                </div>
              )}
              ...
              {/* Complete Button */}
              <Button
                className="h-14 text-lg bg-accent hover:bg-accent/90 text-accent-foreground mt-auto"
